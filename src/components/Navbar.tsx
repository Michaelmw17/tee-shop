...
export default function Navbar({ category }: { category?: string }) {
  ...
  // Beta mode: Simplified navigation
  const showCategoryNav = !BETA_MODE || BETA_FEATURES.showCategoryNav;
    ...
    <nav className="bg-white shadow-sm sticky top-0 z-50 border-b border-gray-100">
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          {/* Beta mode: Simple navigation link */}
          {!showCategoryNav && BETA_FEATURES.showShopTab && (
            <div className="hidden lg:flex justify-center items-center space-x-2 py-4">
              <Link
                href="/store"
                className={`px-4 py-2 rounded font-medium transition-colors ${pathname === "/store" || pathname.startsWith("/store/product") ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-800 hover:bg-blue-100 hover:text-blue-800"}`}
              >Shop</Link>
            </div>
          )}
          ...
          <div className="flex items-center space-x-2">
            {BETA_FEATURES.showCartIcon && (
              <Link href="/store/cart" className="relative p-2 hover:bg-gray-100 rounded-lg transition-colors">
                <ShoppingCart className="h-6 w-6 text-black" />
                {hasMounted && cartCount > 0 && (
                  <span className="absolute -top-2 -right-2.5 bg-red-500 text-white text-xs font-bold rounded-full min-w-[1.25rem] h-5 px-1.5 flex items-center justify-center">
                    {cartCount}
                  </span>
                )}
              </Link>
            )}
            {/* Hamburger only on mobile when needed */}
            {showCategoryNav && (
              <button
